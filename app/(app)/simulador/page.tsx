...
// Redireciona para o simulador dentro da área protegida
// O conteúdo original de page.tsx foi movido para (app)/simulador/page.tsx
...
export default function SimuladorPage() {
    ...
    const [margemPct, setMargemPct] = useState<number>(0);
...
`.trim();
    ...
    }, [match, volume, cliente, base, cidade, uf, rota, impostoTipo, taxaIss, taxaPisCofins, taxaIcms, taxaOutros, impostoFixo, margemPct, zerarImpostos]);

    async function onCopy() {
        if (!calc?.texto) { showToast("Nada para copiar. Complete as seleções.", "warn"); return; }
        try {
    ...
    function onSendEmail() {
        if (!calc?.texto) { showToast("Nada para enviar. Complete as seleções.", "warn"); return; }
    ...
    function onReset() {
        setCliente(""); setEmailDestino(""); setVolume(0); setCidade(""); setBase(""); setUf(""); setRota(""); setImpostoTipo("");
        setTaxaIss(0); setTaxaPisCofins(0); setTaxaIcms(0); setTaxaOutros(0); setImpostoFixo(0); setZerarImpostos(false); setMargemPct(0);
    ...
    function onExample() {
        const r = db[0];
        if (!r) return;
        setCliente("Exemplo • Dependência"); setVolume(25);
        setBase(r.base); setCidade(r.cidade); setUf(r.estado); setRota(r.rota); setImpostoTipo(r.imposto);
        setTaxaIss(2.0); setTaxaPisCofins(1.65); setTaxaIcms(0); setTaxaOutros(2.35); setImpostoFixo(0); setZerarImpostos(false); setMargemPct(15);
        ...
        <>
                ...
                <div className="top">
                    <div className="brand">
                        <div className="mark" aria-hidden="true">
                            ...
                            <img className="logoImg" src="https://www.protege.com.br/media/ovmn4be5/main-logo.svg" alt="Protege" />
                        </div>
                        <div>
                    ...
                    <div className="chips" aria-hidden="true">
                        <div className="chip"><span className="dot"></span> Interface Protege</div>
                        <div className="chip">
                            <span className="dot" style={{ background: "var(--protege-blue-2)", boxShadow: "0 0 0 3px rgba(10,59,138,0.14)" }}></span>{" "}
                            Copy do orçamento
                        </div>
                    </div>
                </div>
                    ...
                    <section className="card">
                        <div className="cardHeader">
                            <div>
                                ...
                                <div className="cardDesc">Defina parâmetros e impostos. O valor final é <b>custo base + tributos</b> com margem opcional.</div>
                            </div>
                        ...
                        <div className="cardBody">
                            <div className="form">
                                <div className="field">
                                    ...
                                    <input value={cliente} onChange={(e) => setCliente(e.target.value)} placeholder="Ex.: Loja 0132 • SUP BH" autoComplete="off" />
                                    <div className="hint">Opcional — aparece no texto final.</div>
                                </div>

                                <div className="field">
                                    <label>E-mail destino</label>
                                    <input type="email" value={emailDestino} onChange={(e) => setEmailDestino(e.target.value)} placeholder="destinatario@empresa.com.br" autoComplete="off" />
                                    <div className="hint">Opcional. Se vazio, abre novo e-mail sem destinatário.</div>
                                </div>

                                <div className="field">
                                    <label>Volume</label>
                                    <input type="number" min={0} step={1} value={volume} onChange={(e) => setVolume(Number(e.target.value))} />
                                    <div className="hint">Quantidade de volumes/atendimentos conforme sua regra.</div>
                                </div>

                                <div className="field">
                                    <label>Cidade</label>
                                    <input value={cidade} onChange={(e) => setCidade(e.target.value)} list="cidadeList" placeholder="Digite para buscar..." autoComplete="off" />
                                    <datalist id="cidadeList">{cidadeOptions.map((c) => <option key={c} value={c} />)}</datalist>
                                ...
                                <div className="field">
                                    ...
                                    <select value={rota} onChange={(e) => setRota(e.target.value)}>
                                        <option value="">Selecione...</option>
                                        {rotaOptions.map((r) => <option key={r} value={r}>{r}</option>)}
                                    </select>
                                ...
                                <div className="field span2">
                                    ...
                                    <select value={impostoTipo} onChange={(e) => setImpostoTipo(e.target.value)}>
                                        <option value="">Selecione...</option>
                                        {impostoOptions.map((i) => <option key={i} value={i}>{i}</option>)}
                                    </select>
                                ...
                                <div className="field"><label>ISS (%)</label><input disabled={zerarImpostos} type="number" step="0.01" value={taxaIss} onChange={(e) => setTaxaIss(Number(e.target.value))} /></div>
                                <div className="field"><label>PIS/COFINS (%)</label><input disabled={zerarImpostos} type="number" step="0.01" value={taxaPisCofins} onChange={(e) => setTaxaPisCofins(Number(e.target.value))} /></div>
                                <div className="field"><label>ICMS (%)</label><input disabled={zerarImpostos} type="number" step="0.01" value={taxaIcms} onChange={(e) => setTaxaIcms(Number(e.target.value))} /></div>
                                <div className="field"><label>Outros (%)</label><input disabled={zerarImpostos} type="number" step="0.01" value={taxaOutros} onChange={(e) => setTaxaOutros(Number(e.target.value))} /></div>
                                <div className="field"><label>Imposto fixo (R$)</label><input disabled={zerarImpostos} type="number" step="0.01" value={impostoFixo} onChange={(e) => setImpostoFixo(Number(e.target.value))} /></div>
                                <div className="field"><label>Margem (%)</label><input type="number" step="0.1" value={margemPct} onChange={(e) => setMargemPct(Number(e.target.value))} /></div>

                                <div className="field span2">
                                    <label className="toggleRow">
                                        <input type="checkbox" checked={zerarImpostos} onChange={(e) => setZerarImpostos(e.target.checked)} />
                                        Zerar impostos (simulação sem tributos)
                                    </label>
                                    <div className="hint">Quando ativado, ISS/PIS/COFINS/ICMS/Outros/Fixo ficam 0 automaticamente.</div>
                                </div>

                                <div className="actions span2 actionsInline">
                                    <button className="primary" onClick={onCopy}>Copiar orçamento</button>
                        ...
                        <div className="cardBody">
                            <div className="kpis">
                                ...
                                <div className="kpi kpiTotal">
                                    ...
                                    <small>Recomendações: {match?.best_days || "-"} • {match?.best_window || "-"}</small>
                                </div>
                            </div>
            ...
            <div className={`toast ${toast.show ? "show" : ""}`}>
                <div className="t" style={{
                    background: toast.kind === "ok" ? "var(--good)" : toast.kind === "warn" ? "var(--warn)" : "var(--bad)",
                    boxShadow: toast.kind === "ok" ? "0 0 0 3px rgba(34,197,94,0.12)" : toast.kind === "warn" ? "0 0 0 3px rgba(245,158,11,0.12)" : "0 0 0 3px rgba(239,68,68,0.12)",
                }} />
